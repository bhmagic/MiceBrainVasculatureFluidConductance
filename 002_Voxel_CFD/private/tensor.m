...
clear 
load('good.mat')
xyz_lower_upper = [4001, 4500;  4001, 4500;  4001, 4500]';
tic



% croping skeleton in the ROI
[xxx,yyy,zzz] = ind2sub(CropSize,S_skel);
flag = all([xxx,yyy,zzz] >= xyz_lower_upper(1,:), 2) & all([xxx,yyy,zzz] <= xyz_lower_upper(2,:),2);
P_xyz = [xxx(flag),yyy(flag),zzz(flag)];
P_sk = S_skel(flag);
P_radii = S_radii(flag);

clear xxx yyy zzz 


% grouping skeleton points into node and links
[P_link_graph_link, P_link, P_link_group, P_wierdNode, P_wierdNode_group] = make_tracible_data(P_sk, P_radii, CropSize);
